...
class hm_framework:
    ...
    def get_matter_consistency(self, exp):
        ...
        I = np.trapz(self.hcos.nzm*self.hcos.bh[:,:,0]*self.hcos.ms/self.hcos.rho_matter_z(0)*mMask,self.hcos.ms, axis=-1)
    ...
    def get_tsz_consistency(self, exp, lmax_proj=None):
        ...
        mMask = np.ones(self.nMasses)
        mMask[exp.massCut < self.hcos.ms] = 0
        I = np.trapz(self.hcos.nzm * self.hcos.bh[:,:,0] * self.hcos.ms / self.hcos.rho_matter_z(0) * mMask, self.hcos.ms,
                     axis=-1)
    ...
    def tsZ_auto_itgrnds_each_z(self, i, ells_out, fftlog_way, get_secondary_bispec_bias, parallelise_secondbispec,
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            if m < exp_minimal.massCut:
                ...
                integ_1h_for_2htrispec[..., j] = y * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_2h_1g[..., j] = np.conjugate(kfft) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_2g[..., j] = phicfft * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_1_3_trispec[..., j] = phicfft * np.conjugate(phicfft_mixed) * hm_minimal.nzm[i, j] * \
                                           hm_minimal.bh[i, j, 0]
            itgnd_2h_2_2_trispec[..., j] = phicfft * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_ky_y[..., j] = np.conjugate(kfft) * phicfft_mixed \
                                    * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            ...
            if get_secondary_bispec_bias:
                ...
                itgnd_2h_second_bispec[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * sec_bispec_rec_2h
    ...
    def tsZ_cross_itgrnds_each_z(self, i, ells_out, fftlog_way, damp_1h_prof, exp_minimal, hm_minimal, survey_name):
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            y = exp_minimal.tsz_filter * tls.pkToPell(hm_minimal.comoving_radial_distance[i], hm_minimal.ks,
                                                      hm_minimal.pk_profiles['y'][i, j])(ells_in)
            itgnd_y_for_2hbispec[..., j] = y * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_2h_1g[..., j] = mean_Ngal * np.conjugate(galfft) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_2g[..., j] = phicfft * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
    ...
    def get_g_cross_kappa(self, exp, survey_name, gzs, gdndz, damp_1h_prof=True, fftlog_way=True, gal_consistency=False):
        ...
        for i,z in enumerate(hcos.zs):
            ...
            for j,m in enumerate(hcos.ms):
                ...
                itgnd_2h_1g[:, j] = mean_Ngal * np.conjugate(galfft)*hcos.nzm[i,j]*hcos.bh[i,j,0]
                itgnd_2h_1m[:, j] = np.conjugate(kfft)*hcos.nzm[i,j]*hcos.bh[i,j,0]
    ...
    def cib_cross_itgrnds_each_z(self, i, ells_out, fftlog_way, damp_1h_prof, exp_minimal, hm_minimal, survey_name):
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_I_for_2hbispec[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (u_cen + u_sat)
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_2h_k[..., j] = mean_Ngal * np.conjugate(galfft) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_II[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] \
    ...
    def cib_auto_itgrnds_each_z(self, i, ells_out, fftlog_way, get_secondary_bispec_bias, parallelise_secondbispec,
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            if m < exp_minimal.massCut:
                ...
                integ_1h_for_2htrispec[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (u_cen + u_sat)
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_2h_k[..., j] = np.conjugate(kfft) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_II[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (
                        phicfft_usat_usat + 2 * phicfft_ucen_usat)
            itgnd_2h_IintIII[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] \
    ...
    def get_cib_ps(self, exp, damp_1h_prof=True, cib_consistency=False):
        ...
        for i,z in enumerate(hcos.zs):
            ...
            for j,m in enumerate(hcos.ms):
                ...
                itgnd_2h_1g[:, j] = hcos.nzm[i, j] * hcos.bh[i, j, 0] * (u_cen + u_sat)
    ...
    def mixed_auto_itgrnds_each_z(self, i, ells_out, fftlog_way, get_secondary_bispec_bias, parallelise_secondbispec,
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            if m < exp_minimal.massCut:
                ...
                integ_1h_I_for_2htrispec[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (u_cen + u_sat)
                integ_1h_y_for_2htrispec[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * y
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_2h_k[..., j] = np.conjugate(kfft) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_Iy[..., j] = (phicfft_ucen_y + phicfft_usat_y) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_yy[..., j] = phicfft_yy * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_2h_II[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (
                        phicfft_usat_usat + 2 * phicfft_ucen_usat)

            itgnd_2h_Iyyy[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (phicfft_Iint_y * phicfft_yy
                                                                                      + 3 * phicfft_yint_y * (
                                                                                                  phicfft_ucen_y
                                                                                                  + phicfft_usat_y))
            itgnd_2h_IIyy[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (
                        2 * phicfft_yy * (phicfft_Iint_ucen
                                          + phicfft_Iint_usat)
                        + 2 * phicfft_yint_y * (2 * phicfft_ucen_usat
                                                + phicfft_usat_usat))
            itgnd_2h_IyIy[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (
                        2 * phicfft_Iint_y * (phicfft_ucen_y
                                              + phicfft_usat_y)
                        + 2 * phicfft_usat_y * (2 * phicfft_Iint_ucen
                                                + phicfft_Iint_usat))
            itgnd_2h_yIII[..., j] = hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0] * (
    ...
    def mixed_cross_itgrnds_each_z(self, i, ells_out, fftlog_way, damp_1h_prof, exp_minimal, hm_minimal, survey_name):
        ...
        for j, m in enumerate(hm_minimal.ms):
            ...
            itgnd_I_for_2hbispec[..., j] = (u_cen + u_sat) * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
            itgnd_y_for_2hbispec[..., j] = y * hm_minimal.nzm[i, j] * hm_minimal.bh[i, j, 0]
